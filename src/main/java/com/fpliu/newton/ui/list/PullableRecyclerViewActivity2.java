 ...
 */
public abstract class PullableRecyclerViewActivity2<T, H extends ItemViewHolderAbs> extends PullableRecyclerViewActivity<T, H> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ItemAdapterEx<T, H> itemAdapter = new ItemAdapterEx<T, H>(null) {
            ...
            public H onCreateHeaderViewHolder(ViewGroup viewGroup, int viewType, View headerView) {
                return PullableRecyclerViewActivity2.this.onCreateHeaderViewHolder(viewGroup, viewType, headerView);
            }

            @Override
            public H onCreateFooterViewHolder(ViewGroup viewGroup, int viewType, View footerView) {
                return PullableRecyclerViewActivity2.this.onCreateFooterViewHolder(viewGroup, viewType, footerView);
            }

            @Override
            public H onCreateViewHolder2(ViewGroup viewGroup, int viewType) {
                return PullableRecyclerViewActivity2.this.onCreateViewHolder(viewGroup, viewType);
            }

            @Override
            public void onBindViewHolder(H holder, int position, T item) {
                PullableRecyclerViewActivity2.this.onBindViewHolder(holder, position, item);
            }
        };
        if (headerView != null) {
