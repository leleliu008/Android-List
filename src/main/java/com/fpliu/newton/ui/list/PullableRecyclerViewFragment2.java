 ...
 */
public abstract class PullableRecyclerViewFragment2<T, H extends ItemViewHolderAbs> extends PullableRecyclerViewFragment<T, H> {
    ...
    protected void onCreateViewLazy(BaseView baseView, Bundle savedInstanceState) {
        ...
        ItemAdapterEx<T, H> itemAdapter = new ItemAdapterEx<T, H>(null) {
            ...
            public H onCreateHeaderViewHolder(ViewGroup viewGroup, int viewType, View headerView) {
                return PullableRecyclerViewFragment2.this.onCreateHeaderViewHolder(viewGroup, viewType, headerView);
            }

            @Override
            public H onCreateFooterViewHolder(ViewGroup viewGroup, int viewType, View footerView) {
                return PullableRecyclerViewFragment2.this.onCreateFooterViewHolder(viewGroup, viewType, footerView);
            }

            @Override
            public H onCreateViewHolder2(ViewGroup viewGroup, int viewType) {
                return PullableRecyclerViewFragment2.this.onCreateViewHolder(viewGroup, viewType);
            }

            @Override
            public void onBindViewHolder(H holder, int position, T item) {
                PullableRecyclerViewFragment2.this.onBindViewHolder(holder, position, item);
            }
        };
        if (headerView != null) {
