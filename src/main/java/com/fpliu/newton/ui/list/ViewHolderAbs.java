...
/**
 * ListView和GridView的Adapter的万能ViewHolder实现（抽象）
 *
...
public abstract class ViewHolderAbs<SubClass> {
    ...
    public ViewHolderAbs(int layoutId, ViewGroup parent) {
    ...
    public static <T> T getInstance(int layoutId, View convertView, ViewGroup parent, Class<T> clazz) {
        if (convertView == null) {
            try {
                Constructor<T> constructor = clazz.getDeclaredConstructor(int.class, ViewGroup.class);
                constructor.setAccessible(true);
                return constructor.newInstance(layoutId, parent);
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
        } else {
            return (T) convertView.getTag(layoutId);
        }
    }
